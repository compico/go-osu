...
export const usePlayerStore = defineStore('player', () => {
    ...
    const pause = (): void => {
        audioService.pause();
        isPlaying.value = false;
        // Сохраняем currentTime при паузе
    ...
    persist: {
        ...
        // В v4+ используется pick вместо paths
        pick: ['currentTrack', 'volume', 'isMuted', 'currentTime', 'playlist'],
        afterHydrate:(ctx) => {
            console.log('[PlayerStore] Restored state:', ctx.store.currentTrack?.title)
        }
    }
});
